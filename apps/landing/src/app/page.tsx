...
import { Bot, Calendar, Bell, FileText, Check, X, MessageSquare, ArrowRight, ChevronDown, Twitter, Facebook, Instagram, Linkedin, Loader2 } from 'lucide-react';

export default function HomePage() {
  const [checkoutOpen, setCheckoutOpen] = useState(false);
  const [openFaqIndex, setOpenFaqIndex] = useState<number | null>(null);
  const [signupStep, setSignupStep] = useState<'phone' | 'otp' | 'name'>('phone');
  ...
  const pricingPlans = [
    {
      name: 'Free',
      price: 0,
      features: ['50 messages/month', '5 reminders/day', 'Basic calendar sync', 'Email support'],
      cta: 'Get Started',
    },
    {
      name: 'Pro',
      price: 29,
      features: ['Unlimited messages', 'Unlimited reminders', 'Advanced calendar sync', 'Document Q&A', 'Voice notes', 'Priority support'],
    ...
    },
    {
      name: 'Enterprise',
      price: 99,
      features: ['Everything in Pro', 'Custom integrations', 'Team collaboration', 'Analytics dashboard', 'Dedicated support', 'SLA guarantee'],
      cta: 'Contact Sales',
    },
  ];
        ...
        <div className="features-container">
          ...
          <div className="features-grid">
            <div className="feature-card">
              <div className="feature-icon" style={{ background: '#dcf8c6', color: '#128C7E' }}>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-header">
              ...
              <p className="modal-subtitle">
                {signupStep === 'phone' && 'Enter your WhatsApp number to get started'}
            ...
            {signupStep === 'otp' && (
              <form onSubmit={handleOtpSubmit} className="modal-form">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (/^\d$/.test(val)) {
                          ...
                          setOtp(newOtp.join(''));
                          if (i < 5) {
                            const nextInput = document.querySelectorAll('.otp-input')[i + 1] as HTMLInputElement;
                            nextInput?.focus();
                          }
                        }
                      }}
                      onKeyDown={(e) => {
                        if (e.key === 'Backspace' && !otp[i] && i > 0) {
                          const prevInput = document.querySelectorAll('.otp-input')[i - 1] as HTMLInputElement;
                          prevInput?.focus();
                        }
                      }}
                      autoFocus={i === 0}
                ...
                <button type="submit" className="modal-button" disabled={isLoading || otp.length < 6}>
                  {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : 'Verify'}
                </button>
                <button type="button" className="modal-resend" onClick={() => setSignupStep('phone')}>
                  Change phone number
                </button>
              </form>
            )}
        ...
        <div className="footer-container">
          <div className="footer-grid">
            <div className="footer-brand-col">
              ...
              <a href="mailto:support@whatsappagent.com" className="footer-email">
